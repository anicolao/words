	...
	for (let co = 0; co < 3; co++) {
		for (const ep of eps) {
			for (let eo = 0; eo < 2; eo++) {
				...
				cc.ep[EDGE] = ep;
				cc.eo[ep] = eo;
				const sol = CachedSolver.get('ss-back')
					.solve(cc, 1, 13, 10)
				...
				result.push(
					`${sols}, back-slot-corner-wrong, DLB=${corner_desc(co, 7)}, BR=${edge_desc(eo, ep)}`
				);
			}
		}
	}
	...
	for (let eo = 0; eo < 2; eo++) {
		for (const cp of cps) {
			for (let co = 0; co < 3; co++) {
				...
				const sols = sol.map((s) => s.sol.toString()).join(' , ');
				result.push(
					`${sols}, back-slot-edge-wrong, DLB=${corner_desc(co, cp)}, BR=${edge_desc(eo, 11)}`
				);
			}
		}
	}
...
it('gens all ss cases', () => {
	...
	const perms = cartesianProduct(eps, eps, cps, [0, 1], [0, 1], [0, 1, 2]).filter(
		(x) => x[0] !== x[1]
	);
	...
	for (const perm of perms) {
		...
		const desc =
			`${sols}, ss-front, dr=${edge_desc(eo1, ep1)}, ` +
	...
	for (const perm of perms) {
		...
		const sols = sol.map((s) => s.toString()).join(' , ');
		const desc =
			`${sols}, ss-back, dr=${edge_desc(eo1, ep1)}, ` +
			`fr=${edge_desc(eo2, ep2)}, dfr=${corner_desc(co1, cp1)}`;
		result.push(desc);
	}
