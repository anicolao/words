...
import { analyze, get_oris, is_fb_solved, prerotate_solves, type AnalyzerState, type SolutionDesc } from '$lib/third_party/onionhoney/Analyzer';
...
describe('optimizer can find helpful shorter solutions', () => {
    function makeSolutionDesc(userSolution: { stage: string; orientation: string; solution: string; }): SolutionDesc {
    ...
    it('can find shorter first block', () => {
        ...
        const computedSolution = orientation + optimized[0][0].premove + ' ' + movesToString(optimized[0][0].solution);
        expect(computedSolution).to.equal("y' x2 x2 R' B' R' M' B' R D2");
        const spin = new MoveSeq(orientation)
        const cube = new CubieCube().apply(scramble).apply(new MoveSeq(computedSolution)).changeBasis(spin).apply(spin.inv());
    ...
    it('can solve ss', () => {
        ...
        const userSS = {stage: 'ss', orientation: orientation, solution: "B' x2 y U' L2 D' L y' u2 f' F U S U2 R"};
        const solutionArray = [ makeSolutionDesc(userFB), makeSolutionDesc(userSS) ];
        const optimized = makeOptimizedData(scramble, solutionArray);
        const computedSolution = userFB.solution + optimized[1][0].premove + ' ' + movesToString(optimized[1][0].solution);
        console.log(computedSolution)
        //expect(computedSolution).to.equal("y' x2 x2 R' B' R' M' B' R D2");
        const spin = new MoveSeq(orientation)
        const cube = new CubieCube().apply(scramble).apply(new MoveSeq(computedSolution)).changeBasis(spin).apply(spin.inv());
        const v = visualize(cube);
