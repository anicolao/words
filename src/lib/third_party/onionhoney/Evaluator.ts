...
export class DPEvaluator extends Evaluator {
	...
	evaluate_with_plan(moves_input: MoveSeq) {
		...
		for (let i = 0; i < len; i++) {
			...
			for (let prev_grip = 0; prev_grip < 4; prev_grip++) {
				for (let shifted_grip = 0; shifted_grip < 4; shifted_grip++) {
					const cur_scheme =
						DP[i][prev_grip] + regripCost[prev_grip][shifted_grip] + moveCost[move][shifted_grip];
		...
		const grip_str = ['HOME', 'R GRIP', "R' GRIP", 'R2 GRIP'];
...
export class MovementEvaluator extends Evaluator {
	...
	evaluate(moves_input: MoveSeq) {
		...
		for (let i = 0; i < moves.length - 1; i++) {
			...
			for (let j = 0; j < 12; j++) {
				if (
					(cube.ep[j] === 5 || cube.ep[j] === 8 || cube.ep[j] === 9) &&
					cube.ep[j] === cube1.ep[j]
				) {
					stationary++;
				}
			}
...
export function getEvaluator(s: string) {
	switch (s) {
		...
		case 'two-gram':
			return new TwoGramEvaluator();
		case 'qtm':
			return new QTMEvaluator();
		case 'dp':
			return new DPEvaluator();
		case 'movement':
