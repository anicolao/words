...
import {
	initialize as min2phase_init,
	solve as min2phase_solve
} from './min2phase/min2phase-wrapper';
...
function Solver(config: SolverConfig): SolverT {
	...
	function generateNextMoveTable() {
		function getAvailableMove(name: string) {
			switch (name[0]) {
				case 'U':
					return moveset.filter((k) => k.name[0] !== 'U');
				case 'D':
					return moveset.filter((k) => k.name[0] !== 'U' && k.name[0] !== 'D');
				case 'R': {
				...
				case 'L':
					return moveset.filter(
						(k) => k.name[0] !== 'R' && k.name[0] !== 'M' && k.name[0] !== 'L' && k.name[0] !== 'r'
					);
				case 'r':
					return moveset.filter(
						(k) => k.name[0] !== 'R' && k.name[0] !== 'M' && k.name[0] !== 'L' && k.name[0] !== 'r'
				...
				case 'F':
					return moveset.filter((k) => k.name[0] !== 'F');
				case 'B':
					return moveset.filter((k) => k.name[0] !== 'F' && k.name[0] !== 'B');

				case 'E':
					return moveset.filter((k) => k.name[0] !== 'U' && k.name[0] !== 'D' && k.name[0] !== 'E');
				case 'S':
					return moveset.filter((k) => k.name[0] !== 'F' && k.name[0] !== 'B' && k.name[0] !== 'S');
				default:
					return moveset;
			}
		}
	...
	function expand(cube: CubieCube, depth: number, solution: Move[]): SState {
		const availableMoves: Move[] =
			solution.length > 0 ? nextMoves[solution[solution.length - 1].name] : moveset;
		let seen_encodings: Set<BigInt | number> | null = null;
		const prune = config.pruneSeenEncodings;
		if (prune) {
			seen_encodings = new Set();
			if (pruners.length === 1) seen_encodings.add(pruners[0].encode(cube));
			else {
				const n =
					BigInt(pruners[0].encode(cube)) * BigInt(pruners[1].size) +
					BigInt(pruners[1].encode(cube));
		...
		for (const move of availableMoves) {
			...
			const enc =
				pruners.length === 1
					? pruners[0].encode(new_cube)
					: BigInt(pruners[0].encode(new_cube)) * BigInt(pruners[1].size) +
					  BigInt(pruners[1].encode(new_cube));
...
const Min2PhaseSolver: () => SolverT = function () {
	...
	function solve(cube: CubieCube, l: number, r: number, c: number) {
		console.assert(
			arrayEqual(cube.tp, new CubieCube().tp),
			'Cube center is not solved: ' + cube.tp
		);
