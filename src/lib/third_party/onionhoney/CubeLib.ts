...
export class CubieCube {
    ...
    static generate_apply_partial_func(oc: Array<OriChg>, pc: Array<PermChg>, mod: number, o: string, p: string) {
        ...
        ${Array(oc.length).fill(0).map( (_, i) => {
            const src = pc[i][0], dst = pc[i][1], ori = oc[i];
            return `
                ${p}_new[${dst}] =  ${p}[${src}];
    ...
    _from_facelet_mapping (fm: [Face[], Face[]][], custom_corners_coord: CornerCoord[], custom_edges_coord: EdgeCoord[], custom_centers_coord: CenterCoord[] ) {
        ...
        const match_pos_piece = (pos: Face[], piece: Face[], coord: Face[][], parity: number) : [number, number, number]=> {
        ...
        for (const coord_pair of fm) {
            const pos = coord_pair[0], piece = coord_pair[1]
            const coord : Face[][] = (pos.length === 2) ? custom_edges_coord : 
    ...
    to_cstimer_cube() {
        ...
        const cube = this._from_facelet_mapping(facelet_mapping, cstimer_corners_coord, cstimer_edges_coord, centers_coord)
        return cube
    }

    to_kstate_cube() {
        const facelet_mapping = this._to_facelet_mapping(corners_coord, edges_coord, centers_coord)
        const cube = this._from_facelet_mapping(facelet_mapping, kcorners_coord, kedges_coord, centers_coord)
        return cube
    }
    ...
    is_solveable() {
        ...
        const ori_correct = (this.co.reduce((x, y) => x + y, 0) % 3 === 0) && (this.eo.reduce((x, y) => x + y, 0) % 2 === 0)
    ...
    changeBasis(s: MoveSeq) {
        ...
        const cube = this._from_facelet_mapping(transformed_mapping, corners_coord, edges_coord, centers_coord)
        return cube
...
export class Move {
    ...
    from_cube(cube: CubieCube, name: string) {
        const get_change = (p: Array<number>, o: Array<number>, acc_p: Array<PermChg>, acc_o: Array<OriChg>) => {
...
export class MoveSeq {
    ...
    inv() {
        const moves: Move[] = this.moves.slice(0).reverse().map(x => x.inv()).flat()
...
const FaceletCube = function () {
    ...
    const color_of_c = (p: number, o1: number, o2: number) =>
    corners_coord[p][(3 - o1 + o2) % 3];
    const color_of_e = (p: number, o1: number, o2: number) =>
...
const CubeUtil = (() => {
    ...
    const is_solved = (cube: CubieCube, mask: Mask) => {
        ...
        const c_true = co.every( (_, i) =>  (cp[i] === 0 || cube.cp[i] === i) 
                        && (co[i] === 0 || cube.co[i] === 0) )
        if (!c_true) return false
        const e_true = eo.every( (_, i) =>  (ep[i] === 0 || cube.ep[i] === i) 
        && (eo[i] === 0 || cube.eo[i] === 0) )
    ...
    const stickers = [ ...ext(u_face, U), ...ext(d_face, D),
    ...
    const find_pairs = function() {
        ...
        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const ep_stickers : StickerExtT[][] = Array(12).fill(0).map(_ => Array(0))
        edge_stickers.map(s => ep_stickers[s[0]].push(s) )

        // eslint-disable-next-line @typescript-eslint/no-unused-vars
        const cp_stickers : StickerExtT[][] = Array(8).fill(0).map(_ => Array(0))
    ...
    const is_mask_solved2 = (cube: CubieCube, { co, eo, cp, ep }: Mask, premove: (Move | Move[])[]) => {
...
export class ColorScheme extends Storage {
    ...
    getColorsForOri(s: string) {
        const faces = (ColorScheme.valid_schemes_map.get(s) || ColorScheme.valid_schemes_map.get("WG")) + "X";
        const arr : string[] = []
